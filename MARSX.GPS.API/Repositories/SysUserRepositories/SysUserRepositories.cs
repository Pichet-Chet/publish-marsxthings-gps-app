...
{
    public class SysUserRepositories : ISysUserRepositories
    {
        private readonly TrackerContext _context;

        private readonly SysUserService service;

        public SysUserRepositories()
        {
            _context = new TrackerContext();

            service = new SysUserService(_context);
        }
        ...
        public async Task<Response> UpdatePassCode(SysUser param)
        {
            ...
            try
            {
                resp = await Task.Run(() => service.UpdatePassCode(param));
            }
            catch (Exception ex)
            {
                resp.httpCode = Constants.httpCode500;
