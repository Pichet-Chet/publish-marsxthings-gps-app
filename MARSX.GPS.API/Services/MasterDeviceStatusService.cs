...
{
	public class MasterDeviceStatusService
	{
        private readonly TrackerContext _context;

        public MasterDeviceStatusService(TrackerContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(MasterDeviceStatusFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterDeviceStatuses.AsQueryable());
                ...
                List<MasterDeviceStatus> execute = new List<MasterDeviceStatus>();
        ...
        public async Task<Response> Detail(int id)
        {
            ...
            MasterDeviceStatus objData = new MasterDeviceStatus();

            try
            {
                var queryable = await Task.Run(() => _context.MasterDeviceStatuses.Where(x => x.Id == id).AsQueryable());
        ...
        public async Task<Response> Create(MasterDeviceStatus param)
        {
            ...
            try
            {
                var objDuplicate = await Task.Run(() => _context.MasterDeviceStatuses.Where(x => x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
                ...
                if (objDuplicate == null)
                {
                    ...
                    await _context.MasterDeviceStatuses.AddAsync(param);
        ...
        public async Task<Response> Update(MasterDeviceStatus param)
        {
            ...
            try
            {
                var objUpdate = await Task.Run(() => _context.MasterDeviceStatuses.Where(x => x.Id == param.Id).FirstOrDefault());

                if (objUpdate != null)
                {
                    var checkDuplicate = await Task.Run(() => _context.MasterDeviceStatuses.Where(x => x.Id != param.Id && x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
