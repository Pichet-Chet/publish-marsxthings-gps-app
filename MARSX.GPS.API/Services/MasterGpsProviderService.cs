...
{
	public class MasterGpsProviderService
	{
        private readonly TrackerContext _context;

        public MasterGpsProviderService(TrackerContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(MasterGpsProviderFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterGpsProviders.AsQueryable());
                ...
                List<MasterGpsProvider> execute = new List<MasterGpsProvider>();
        ...
        public async Task<Response> Detail(int id)
        {
            ...
            MasterGpsProvider objData = new MasterGpsProvider();

            try
            {
                var queryable = await Task.Run(() => _context.MasterGpsProviders.Where(x => x.Id == id).AsQueryable());
        ...
        public async Task<Response> Create(MasterGpsProvider param)
        {
            ...
            try
            {
                var objDuplicate = await Task.Run(() => _context.MasterGpsProviders.Where(x => x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
                ...
                if (objDuplicate == null)
                {
                    ...
                    await _context.MasterGpsProviders.AddAsync(param);
        ...
        public async Task<Response> Update(MasterGpsProvider param)
        {
            ...
            try
            {
                var objUpdate = await Task.Run(() => _context.MasterGpsProviders.Where(x => x.Id == param.Id).FirstOrDefault());

                if (objUpdate != null)
                {
                    var checkDuplicate = await Task.Run(() => _context.MasterGpsProviders.Where(x => x.Id != param.Id && x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
