...
{
	public class MasterThaiDistrictsService
	{
        private readonly TrackerContext _context;

        public MasterThaiDistrictsService(TrackerContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(MasterThaiDistrictFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterThaiDistricts.AsQueryable());

                #region Filter Data

                if (param.ProvinceId != null)
                {
                    queryable = queryable.Where(x => x.ProvinceId == param.ProvinceId).AsQueryable();
                ...
                {
                    queryable = queryable.Where(x => x.ProvinceId == null).AsQueryable();
                }

                List<MasterThaiDistrict> execute = new List<MasterThaiDistrict>();
