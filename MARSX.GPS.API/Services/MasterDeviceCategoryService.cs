...
{
    public class MasterDeviceCategoryService
    {
        private readonly TrackerContext _context;

        public MasterDeviceCategoryService(TrackerContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(MasterDeviceCategoryFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterDeviceCategories.AsQueryable());
                ...
                List<MasterDeviceCategory> execute = new List<MasterDeviceCategory>();

                execute = queryable.AsNoTracking().ToList();

                execute = execute.OrderBy(x => x.NameEn).ToList();
        ...
        public async Task<Response> Detail(int id)
        {
            ...
            MasterDeviceCategory objData = new MasterDeviceCategory();

            try
            {
                var queryable = await Task.Run(() => _context.MasterDeviceCategories.Where(x => x.Id == id).AsQueryable());
        ...
        public async Task<Response> Create(MasterDeviceCategory param)
        {
            ...
            try
            {
                var objDuplicate = await Task.Run(() => _context.MasterDeviceCategories.Where(x => x.NameEn.ToLower() == param.NameEn.ToLower() && x.NameTh.ToLower() == param.NameTh.ToLower()).FirstOrDefault());
                ...
                if (objDuplicate == null)
                {
                    ...
                    await _context.MasterDeviceCategories.AddAsync(param);
        ...
        public async Task<Response> Update(MasterDeviceCategory param)
        {
            ...
            try
            {
                var objUpdate = await Task.Run(() => _context.MasterDeviceCategories.Where(x => x.Id == param.Id).FirstOrDefault());

                if (objUpdate != null)
                {
                    var checkDuplicate = await Task.Run(() => _context.MasterDeviceCategories.Where(x => x.Id != param.Id && x.NameEn.ToLower() == param.NameEn.ToLower() && x.NameTh.ToLower() == param.NameTh.ToLower()).FirstOrDefault());
