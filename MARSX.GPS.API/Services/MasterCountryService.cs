...
namespace MARSX.GPS.API.Services
{
	public class MasterCountryService
	{
        ...
        public MasterCountryService(TrackerContext context)
        {
            _context = context;
        }

        public async Task<Response> Get(MasterThaiDistrictsFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterCountries.AsQueryable());
                ...
                if (!string.IsNullOrWhiteSpace(param.textSearch))
                {
                    ...
                    x.Code.ToLower().Contains(param.textSearch.ToLower()) ||
                    x.CurrencyCode.ToLower().Contains(param.textSearch.ToLower()) ||
                ...
                {
                    queryable = queryable.Where(x => x.Code.ToLower().Contains(param.Code.ToLower())).AsQueryable();
                }

                if (!string.IsNullOrEmpty(param.CurrencyCode))
                {
                    queryable = queryable.Where(x => x.CurrencyCode.ToLower().Contains(param.CurrencyCode.ToLower())).AsQueryable();
                }
                ...
                List<MasterCountry> execute = new List<MasterCountry>();
