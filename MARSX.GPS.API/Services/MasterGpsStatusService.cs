...
{
	public class MasterGpsStatusService
	{
        private readonly TrackerContext _context;

        public MasterGpsStatusService(TrackerContext context)
        {
            _context = context;
        }


        public async Task<Response> Get(MasterGpsStatusFilter param) // Additional models are imported from GlobalFilter and PaginationModel
        {
            Response resp = new Response();

            try
            {
                var queryable = await Task.Run(() => _context.MasterGpsStatuses.AsQueryable());
                ...
                List<MasterGpsStatus> execute = new List<MasterGpsStatus>();
        ...
        public async Task<Response> Detail(int id)
        {
            ...
            MasterGpsStatus objData = new MasterGpsStatus();

            try
            {
                var queryable = await Task.Run(() => _context.MasterGpsStatuses.Where(x => x.Id == id).AsQueryable());
        ...
        public async Task<Response> Create(MasterGpsStatus param)
        {
            ...
            try
            {
                var objDuplicate = await Task.Run(() => _context.MasterGpsStatuses.Where(x => x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
                ...
                if (objDuplicate == null)
                {
                    ...
                    await _context.MasterGpsStatuses.AddAsync(param);
        ...
        public async Task<Response> Update(MasterGpsStatus param)
        {
            ...
            try
            {
                var objUpdate = await Task.Run(() => _context.MasterGpsStatuses.Where(x => x.Id == param.Id).FirstOrDefault());

                if (objUpdate != null)
                {
                    var checkDuplicate = await Task.Run(() => _context.MasterGpsStatuses.Where(x => x.Id != param.Id && x.NameEn == param.NameEn && x.NameTh == param.NameTh).FirstOrDefault());
