...
namespace MARSX.GPS.API.Controllers
{
    ...
    {
        private readonly ITcDevicesRepositories repoCollection;

        public TcDevicesController()
        {
            repoCollection = new TcDevicesRepositories();
        }

        [HttpGet]
        [Route("Get")]
        //[Authorize]
        public async Task<IActionResult> Get([FromQuery] DevicesFilter param)
        {
        ...
        [HttpGet]
        [Route("Model/{model}")]
        public async Task<IActionResult> DevicesByModel(string model)
        {
            Response result = new Response();
            result = await repoCollection.DevicesByModel(model);
            return StatusCode(result.httpCode, AppHelper.GetResponseController(result));

        }
        [HttpGet]
        [Route("Group/{id}")]
        public async Task<IActionResult> DevicesByGroup(int id)
        {
            Response result = new Response();
            result = await repoCollection.DevicesByGroup(id);
            return StatusCode(result.httpCode, AppHelper.GetResponseController(result));
